defmodule TriggersWeb.AuthController do
  require Logger
  use TriggersWeb, :controller
  alias Triggers.Data
  alias Triggers.Data.User
  alias TriggersWeb.AuthPlugs
  ...
  def signup_submit(conn, %{"user" => user_params}) do
    case Data.insert_user(user_params, :owner) do
      {:ok, user} ->
        ...
        Triggers.Emails.confirm_address(user, user.email) |> Triggers.Mailer.send()
  ...
  def request_password_reset_submit(conn, %{"user" => %{"email" => email}}) do
    if user = Repo.one(User.filter(email: email)) do
      Triggers.Emails.reset_password(user) |> Triggers.Mailer.send()
